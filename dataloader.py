...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.args.sample_method == 'uni':
            ...
            ##################
        return positive_sample, negative_sample, subsampling_weight, mask, self.mode, self.args.sample_method, idx
    ...
    def collate_fn(data):
        ...
        mode = data[0][4]
        idx = torch.LongTensor([_[6] for _ in data])  # 提取 idx
        return positive_sample, negative_sample, subsample_weight, mask, mode, idx
        ...
        for k in f.keys():
            ...
            try:
                if sentence_num >= 4:
                    ...
                    ent_text_emb[ent2id[ent_link['http://dbpedia.org/resource/' + k]]][
                    :sentence_num] = torch.from_numpy(v)
            except KeyError:
                text_count += 1
...
def Emb_Kuai16K(args, ent2id, device):
    import h5py
    import numpy as np
    ent_text_emb = torch.zeros([len(ent2id), 4, 384], device=device)
    ent_img_emb = torch.zeros([len(ent2id), 24, 383], device=device)
